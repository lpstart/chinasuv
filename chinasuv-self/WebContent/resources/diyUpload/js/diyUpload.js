 ...
 */
(function($) {
	var $coverInputId = "#coverImg";

	$.fn.extend({
		/*
		 * 上传方法 opt为参数配置; serverCallBack回调函数 每个文件上传至服务端后,服务端返回参数,无论成功失败都会调用
		 * 参数为服务器返回信息;
		 */
		diyUpload : function(opt, serverCallBack) {
			...
			webUploader.on('uploadSuccess', function(file, response) {
				var d = eval(response);
				var $fileBox = $('#fileBox_' + file.id);
				$fileBox.attr("imgId",d.id);
				if($coverInputId!=("#" + $fileInput.attr('id'))){
					$fileBox.append('<div class="descDiv descriptionDiv'+d.id+'" id="'+d.id+'">添加描述</div>');
					$(".descriptionDiv"+d.id).click(descDialog)
				}
				var $diyBar = $fileBox.find('.diyBar');
				if($coverInputId==("#" + $fileInput.attr('id'))){
	...
	function descDialog() {
		...
		dialog({
					title : '输入描述信息：',
					content : '<textarea autofocus id="description">'
							+ rowDesc + '</textarea>',
					cancel : function() {
						this.close();
					},
					ok : function() {
						$.ajax({
							...
							data: "imgId="+imgIdInDB+"&description="+$('#description').val(),
							dataType : "JSON",
							contentType: "application/x-www-form-urlencoded; charset=utf-8",
							async: true,
							success:function(data){
	...
	function getOption(objId) {
		...
		if (objId == $coverInputId) {
			tempValue.pick.label = "点击选择封面";
			tempValue.fileNumLimit = 1;
		}
		return tempValue;
	}
	...
	function createBox($fileInput, file, webUploader) {
		...
		// 绑定取消事件;
		var $diyCancel = $fileBox.children('.diyCancel').one('click',
				function() {
